...
(define scheme-expr?
  (lambda (exp)
    (or (atomic? exp) (special-form? exp) (application? exp))))
...
(define or-last-arg?
  (lambda (exp)
    (null? (cdr (get-content exp)))))    

(define or-empty-args?
  (lambda (exp)
    (null? (get-content exp))))

(define make-or
  (lambda (conditions)
    (attach-tag conditions 'or)))    
...
(define derived?
  (lambda (exp)
    (or (cond? exp) (function-definition? exp) (let? exp) (letrec? exp) (or? exp) )))
...
(define or->if
  (lambda (exp)
    (if (or-empty-args? exp) '#f
      (make-if (or-first-arg exp) (or-first-arg exp) (make-or (or-rest-args exp)) )
      ) ))
...
; Translation of let into a box-based let
;(define letrec->let
;  (lambda (exp)
;    (let* ((vars (letrec-variables exp))
