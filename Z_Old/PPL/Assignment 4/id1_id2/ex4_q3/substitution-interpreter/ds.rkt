...
;Signature: get-variable-exists(sub,var)
;Type: Client view: [Sub*Var*[Empty->T] -> Evaluator-value]
;      Supplier view: [LIST*Symbol*[Empty->T] ->  Symbol] 
;Purpose: If sub is an empty substitution or does not includes var, fail-cont is applied.
(define get-variable-exists
  (lambda (sub var)
    (letrec ((lookup
              (lambda (vars vals)
                (cond ((empty? vars)
                       '(value #f))
                      ((eq? var (car vars)) '(value #t))
                      (else (lookup (cdr vars) (cdr vals)))))))
      (lookup (get-variables sub) (get-values sub)))
  ))
