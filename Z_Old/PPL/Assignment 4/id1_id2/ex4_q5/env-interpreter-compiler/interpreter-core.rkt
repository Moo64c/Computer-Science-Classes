...
(define eval-special-form
  (lambda (exp env)
    (cond ((quoted? exp) (text-of-quotation exp))
          ...
          ((exists?? exp) (eval-exists? exp env))
          )))


;; Signature: eval-­exists?(exp) 
;; Purpose: checks is a variable is bounded to a value in the global 
;; environment. 
;; Examples:
...
(define eval-exists?
  (lambda (exp env)
    (lookup-variable-exists env (var-exists? exp))))
