...
(define (if-tests)
  ...
  (test (derive-eval '(f1 5)) => 40)
  (test (derive-eval '(f2 5)) => 120))
...
(define (Box-tests)
  ...
  (test (derive-eval '(define f3 (let ((x (box 0)))
                                  (lambda () (set-box! x (+ (unbox x) 1)) (unbox x))))) => 'ok)
  (test (derive-eval '(f3)) => 1)
  (test (derive-eval '(f3)) => 2)
  (test (derive-eval '(begin (f3) (f3) (f3) (f3))) => 6)
  )
