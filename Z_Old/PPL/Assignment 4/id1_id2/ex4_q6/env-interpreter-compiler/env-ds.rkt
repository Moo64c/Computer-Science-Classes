...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Variadic-Closure:
; Type: [Symbol*LIST*Env -> LIST]
(define make-variadic-procedure
  (lambda (parameter body env)
    (attach-tag (list parameter body env) 'variadic-procedure)))

; Type: [T -> Boolean]
(define compound-variadic-procedure?
  (lambda (p)
    (tagged-by? p 'variadic-procedure)))

; Type: [LIST -> Symbol]
(define variadic-procedure-parameters
  (lambda (p)
    (car (get-content p))))

; Type: [LIST -> LIST]
(define variadic-procedure-body
  (lambda (p)
    (cadr (get-content p))))

; Type: [LIST -> Env]
(define variadic-procedure-environment
  (lambda (p)
    (caddr (get-content p))))    
