...
(define analyze-special-form 
  (lambda (exp)
    (cond ((quoted? exp) (analyze-quoted exp))
	  ( (lambda-variadic? exp) (analyze-variadic-lambda exp) )
...
;;;;;;;;;;;;;;;;;;;;;;;;;OUR CODE;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define analyze-variadic-lambda 
  (lambda (exp)
    (let ((parameters (lambda-variadic-parameters exp))
          (body (analyze-sequence (lambda-variadic-body exp))))
      (lambda (env)
        (make-variadic-procedure parameters body env)))))        
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;OUR CODE;;;;;;;;;;;;;;;;;;;;;;;;;;;;
          ((compound-variadic-procedure? procedure)
           (let* ((parameter (variadic-procedure-parameters procedure))
                  (body (variadic-procedure-body procedure))
                  (env (variadic-procedure-environment procedure))
                  (new-env  (extend-env (make-frame (list parameter) (list arguments) ) env)))
             (body new-env)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define apply-primitive-procedure 
  (lambda (proc args)
    (apply (primitive-implementation proc) 
           (map (lambda (arg)
                   (cond ((primitive-procedure? arg) (primitive-implementation arg))
