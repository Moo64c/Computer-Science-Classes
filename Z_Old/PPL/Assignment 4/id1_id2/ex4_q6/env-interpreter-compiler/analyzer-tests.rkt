...
(define (if-tests)
  ...
  (test (derive-analyze-eval '(define (f1 n) (if (= n 0) 1 (* n (f (- n 1)))))) => 'ok)
  (test (derive-analyze-eval '(define (f2 n) (if (= n 0) 1 (* n (f2 (- n 1)))))) => 'ok)
  (test (derive-analyze-eval '(f1 5)) => 40)
  (test (derive-analyze-eval '(f2 5)) => 120))
