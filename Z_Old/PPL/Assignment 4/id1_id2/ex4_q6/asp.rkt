...
(define special-form?
  (lambda (exp)
    (or (quoted? exp) (lambda? exp) (definition? exp)
        (if? exp) (begin? exp) (assignment? exp) (lambda-variadic? exp) )))
...
(define lambda-variadic?
  (lambda (exp)
    (and (tagged-by? exp 'lambda) (not (list? (cadr exp)) ))))

(define lambda-variadic-parameters
  (lambda (exp)
    (car (get-content exp))))
...
(define make-lambda-variadic
  (lambda (parameters body)
    (attach-tag (cons parameters body) 'lambda-variadic)))    
