...
% true.
%
% ?- s([a, funny, beautiful, cat, in, the, tall, beautiful, house, saw, a,
%       dog]).
% false.
%
% ?- s([house, the, cat, saw, a, dog]).
% false.
%
% ?- s([a, cat, ate, it]).
% false.
...
vp(Z)        :- append(A, Y, Z), append(V, X, A), np(X), v(V), pp(Y).
adjs([]).
adjs([X|Xs]) :- adj([X]), adjs(Xs).
pp([]).
pp(Z)        :- append(X, Y, Z), p(X), np(Y).
adj([Z])     :- member(Z, [beautiful, funny , tall, big]).
det([Z])     :- member(Z, [a, the]).
n([Z])       :- member(Z, [cat, dog, mouse, house, table]).
v([Z])       :- member(Z, [saw, ate]).
p([Z])       :- member(Z, [in, from, on, with]).
