...
runCheck(Pred, Form, Answers) :- mynl,
                                 timeout(Timeout),
...
sortedCheck(Pred, Form, Answers) :- mynl,
                                    timeout(Timeout),
                                    call_with_time_limit(Timeout,
                                        findall(Form, Pred, List)),
                                    sort(List, Sorted),
...
sorteachCheck(Pred, Form, Answers) :- mynl,
                                      timeout(Timeout),
                                      call_with_time_limit(Timeout,
                                          findall(Form, Pred, List)),
                                      sortEach(List, SortedList),
                                      sortEach(Answers, SortedAnswers),
                                      inform(quick_permutation(SortedList,
                                            SortedAnswers), list(Answers),
                                            list(List)),
                                      !.

firstAnswer(Pred, Form, Answer) :- mynl,
                                   timeout(Timeout),
...
firstAnswerUnify(Pred, Form, Answer) :- mynl,
                                        timeout(Timeout),
                                        (call_with_time_limit(Timeout, Pred) ->
                                        !,
                                        inform(equal(Form, Answer), Answer, Form);
                                        inform(Answer)).
...
now :- get_time(X),
       stamp_date_time(X, date(_, _, _, A, B, _, _, _, _), local),
       write(A),
       write(:),
       (B < 10 -> write(0) ; otherwise),
       write(B).
