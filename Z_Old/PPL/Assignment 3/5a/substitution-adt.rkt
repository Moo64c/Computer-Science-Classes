...
;; A substitution is represented as a 2 selection lambda, with two pieces of info:
;; lists of variables and type expression.
;; The empty substitution is the lambda that returns null lists for both var and val.
...
;;       Supplier view: [LIST(Symbol)*LIST(LIST union Symbol)) -> [[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST]]
  ...
  (lambda (variables tes)
    ...
    (if (ormap occur? variables tes)
        (error 'make-sub "circular substitution: variables are ~s, tes are ~s"
               variables tes)
        (lambda (sel) (sel variables tes 'sub) ))))
...
;;       Supplier view: [[[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST] -> LIST(Symbol)]
...
;;       Supplier view: [[[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST] -> LIST(LIST union Symbol)]
;; Example: (get-tes
;;           (make-sub '(x y z) (map te-parse '(T Boolean (T -> Number)))))
;;          ==> (T Boolean (-> (* T) Number))
(define sub->tes
...
;;       Supplier view: [[[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST] -> LIST union Symbol]
...
;;       Supplier view: [[[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST] * Symbol * LIST union Symbol -> [[LIST(Symbol)*LIST(LIST union Symbol)*Symbol->LIST union Symbol]->LIST]]
;; Examples:
;; (extend-sub
;;   (make-sub '(T1 T2 T3)
...
(define sub?
  (lambda (tsub)
    (and (procedure? tsub)
         (= (procedure-arity tsub) 1)
         (eq? (tsub (lambda (x y z) z)) 'sub) 
         (let ((vars (tsub (lambda (x y z) x)))
                      (tes (tsub (lambda (x y z) y))))
                  (and (list? vars)
                       (andmap symbol? vars)
                       (list? tes)
                       (andmap te? tes)
