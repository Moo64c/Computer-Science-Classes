...
(define special-form?
  (lambda (exp)
    (or (quoted? exp) (lambda? exp) (definition? exp)
        (if? exp) (begin? exp) (assignment? exp) (initialize-counter? exp))))
...
(define initialize-counter? (lambda (exp) 
  (tagged-by? exp initialize-counter?)))

(define make-initialize-counter (lambda (exp)
  (attach-tag exp 'initialize-counter)))
