...
public class HttpTokenizer implements MessageTokenizer<HttpMessage> {
   ...
   private final HttpEncoder _httpEncoder;

   public HttpTokenizer(String separator, Charset charset) {
      this._messageSeparator = separator;

      this._decoder = charset.newDecoder();
      this._encoder = charset.newEncoder();
      _httpEncoder = new HttpEncoder();
   }
   ...
   public synchronized HttpMessage nextMessage() {
      ...
      return _httpEncoder.toObject(message);
  }
