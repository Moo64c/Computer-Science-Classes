...
void printString(const vector<string> &toPrint) {
	for(int i =0; i < (int) toPrint.size(); i++) {
		cout << toPrint[i].c_str() << endl;
	}
}

void printRoads(vector<road> *roads) {
	cout << "Road\tfrom\tto\tlength\tstress\tappearances" << endl;

	for(int i =0; i< (int) roads->size(); i++) {
		road current = roads->at(i);
		cout << i+1 << " \t" << current.from.c_str() << "\t" << current.to.c_str();
		cout << "\t"<< current.length << "\t" << current.stress << "\t";
		cout << current.appearances << endl;
	}
}
