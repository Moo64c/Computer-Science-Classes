...
/* Detect viruses in infected file */
 ...
 free(filecontent);
 filecontent = (unsigned char*) malloc(10000);
 fsize = fsize > 10000 ? 10000 : fsize;
 fread(filecontent, fsize, 1, rm);
	...
	for(i =0; i < size; i++) {
		...
		while(v_run != &empty) {
			if ((*(v_run->signature)) == (*(buffer+i))) {
				/* should just use memcmp() */
				result = memcmp(v_run->signature, buffer+i, v_run->length);
				if (result == 0) {
					printf("Found a match at 0x%x\n", (i));
					printf("To virus %s\n", v_run->name);
					printf("Virus size: %i\n\n", v_run->length);
				}
			}
