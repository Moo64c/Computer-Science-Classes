...
void execute(cmdLine *pCmdLine) {
  ...
  if (pCmdLine->next != NULL) {
  	return execute_with_pipe(pCmdLine);
  }
  ...
  if (env_vars != NULL) {
	...
	if ((pid = fork()) < 0) {     /* fork a child process  */
		...
		if (pCmdLine->inputRedirect != NULL) {
			fclose(stdin);
			file_ph = fopen(pCmdLine->inputRedirect, "r");
		}

		if (pCmdLine->outputRedirect != NULL) {
			fclose(stdout);
			file_ph = fopen(pCmdLine->outputRedirect, "w");
		}
...
void execute_with_pipe(cmdLine *pCmdLine) {
  ...
  /**
   fork #1 
	 - move output to child #2	
	 - get input from file if releveant
  */
    fclose(stdout);
    current = dup(_pipe[1]);
    close(_pipe[1]);
    ...
    current = execvp(pCmdLine->arguments[0], pCmdLine->arguments);
  }
  else {
    close(_pipe[1]);   
    if ((pid2 = fork()) == -1) {
     ...
     */
      ...
      if (pCmdLine->next->outputRedirect != 0) {
				fclose(stdout);
				file_ph = fopen(pCmdLine->next->outputRedirect, "w");
      }
      
      current = execvp(pCmdLine->next->arguments[0], pCmdLine->next->arguments);
    }
