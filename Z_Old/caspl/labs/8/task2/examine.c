...
#define DYNAMIC_SYMBOL_TABLE_NAME ".dynsym"
#define SYMBOL_STRING_TABLE_NAME ".strtab"
#define DYNAMIC_SYMBOL_STRING_TABLE_NAME ".dynstr"
...
Elf32_Sym *get_dynamic_symbol_table(void *map_start, Elf32_Shdr *s_header, char *str_table, int num_of_headers)
{ 
  s_header = get_section_table_pointer(map_start, s_header, DYNAMIC_SYMBOL_TABLE_NAME, str_table, num_of_headers);
  return (Elf32_Sym *) (map_start + s_header->sh_offset);
}
...
char *get_dynamic_symbol_string_table(void *map_start, Elf32_Shdr *s_header, char *str_table, int num_of_headers)
{
  s_header = get_section_table_pointer(map_start, s_header, DYNAMIC_SYMBOL_STRING_TABLE_NAME, str_table, num_of_headers);
  return (char *) (map_start + s_header->sh_offset);
}
...
void read_dynamic_symbol_table_entities(void *map_start, int num_of_headers, int offset, int section_size,  int sh_str_table_index)
{
  char *str_table, *sym_str_table;
  Elf32_Sym *sym_table;
  int num_of_symtable_entities;
  ...
  Elf32_Shdr *s_header = (Elf32_Shdr *) ( map_start + offset);
  str_table = get_string_table(map_start, s_header, sh_str_table_index);
  sym_table = get_dynamic_symbol_table(map_start, s_header, str_table, num_of_headers);
  sym_str_table = get_dynamic_symbol_string_table(map_start, s_header, str_table, num_of_headers);
  
  
  symtab_header = get_section_table_pointer(map_start, s_header, DYNAMIC_SYMBOL_TABLE_NAME, str_table, num_of_headers);
...
int main(int argc, char **argv) {
  ...
  read_dynamic_symbol_table_entities(map_start, header->e_shnum, header->e_shoff, header->e_shentsize, header->e_shstrndx);
